 ...
 */
public class Player05 implements NewPlayer {
    /**
     ...
     * @param name
     */
    public Player05(String name, int sampleSize, double percentage, Strategy strategy) {
        this.name = name;
    ...
    public GameMove getMove(GameMove lastMove, boolean maximize, boolean includeAngles) {
        if (maximize) {
            ...
            switch (strategy) {
                case BruteForce:
                    return getBruteForceMove(maximize, includeAngles);
                case Mirroring:
            ...
            return getMinimizingMove(includeAngles);
        }
    }
    ...
    public GameMove getMinimizingMove(boolean includeAngles) {
        ...
        if (enemyCounterMinimzer()) {
            lastOwnMove = null;
            return getBruteForceMove(false, includeAngles);
        }
        ...
        lastOwnMove = null;
        return getBruteForceMove(false, includeAngles); // Found no easy move, do some random stuff and try again
    }

    public GameMove getBruteForceMove(boolean maximize, boolean includeAngles) {
        ...
        double bestCrossingsAddedByVertex = maximize ? -9999999 : 9999999;
        int bestSample = 0;
        ...
        betterEdgeCrossingRTree.insertAllCoordinates(gs.getPlacedVertices());

        // System.out.println("Crossings: " + betterEdgeCrossingRTree.testCoordinate(v,
        // coordinateToAdd, gs.getVertexCoordinates()));
        ...
        // System.out.println("Crossing calc: " + x.computeCrossingNumber() + "
        // crossings in round: " + gs.getPlacedVertices().size());
