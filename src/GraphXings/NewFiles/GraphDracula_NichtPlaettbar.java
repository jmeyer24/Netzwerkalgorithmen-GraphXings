 ...
 */
public class GraphDracula_NichtPlaettbar implements NewPlayer {
    /* ---------------- attributes -------------- */
    /**
    ...
    /**
     * The percentage value for some strategies. See {@link MaximizingStrategy}
     * 
     * @TODO make percentage a variable for runtime maximization. See
     *       {@link #getBruteForceMove(GameMove, Role) BruteForce}
     */
    private double percentage;
    /**
     * The percentage value for strategy {@code RadialStuff}.
     */
    private int fieldPercentage = 17;
    /**
    ...
    /**
     * The {@link MaximizingStrategy} used
     */
    private MaximizingStrategy maxStrat;
    /**
     * The {@link MinimizingStrategy} used
     */
    private MinimizingStrategy minStrat;
    /**
     * Check if we have a small game board and switch maxStrat
     */
    private boolean useSmallBoardStrategy;
    /**
    ...
    /* ---------------- constructor -------------- */
    /**
     * The default constructor for {@code GraphDracula_NichtPlaettbar}
     */
    public GraphDracula_NichtPlaettbar() {
        this.name = "GraphDracula_NichtPlaettbar";
        this.sampleSize = 30;
        this.vertexSampleSize = 1;
        this.percentage = 0.93;
        this.fieldPercentage = 17;
        this.relativeCircleSize = 0.5;
        this.maxStrat = MaximizingStrategy.Mirroring;
        this.minStrat = MinimizingStrategy.EdgeDirectionMean;
        this.r = new Random(this.name.hashCode());
    }

    /**
     * Constructs a {@code GraphDracula_NichtPlaettbar} with the specified
     * strategies
     *
     * @param maxStrat the {@link MaximizingStrategy} we choose
     * @param minStrat the {@link MinimizingStrategy} we choose
     */
    public GraphDracula_NichtPlaettbar(MaximizingStrategy maxStrat, MinimizingStrategy minStrat) {
        this.sampleSize = 30;
        this.vertexSampleSize = 1;
        this.percentage = 0.93;
        this.fieldPercentage = 17;
        ...
        this.name = "GraphDracula_NichtPlaettbar_" + maxStrat + "_" + minStrat;
        this.r = new Random(this.name.hashCode());
    }

    /**
     * Constructs a {@code GraphDracula_NichtPlaettbar} with the specified initial
     * attribute
     ...
     *                           value (see enum MaximizingStrategy documentation)
     ...
     * @param minStrat           the {@link MinimizingStrategy} we choose
     */
    public GraphDracula_NichtPlaettbar(double percentage, double relativeCircleSize, int sampleSize,
            int vertexSampleSize,
            MaximizingStrategy maxStrat, MinimizingStrategy minStrat) {
        ...
        // fixed attributes
        this.name = "GraphDracula_NichtPlaettbar_" + maxStrat + "_" + minStrat + "_" + percentage + "_"
                + relativeCircleSize + "_"
                + sampleSize + "_" + vertexSampleSize;
        this.r = new Random(this.name.hashCode());
    }
    ...
    public GameMove maximizeCrossings(GameMove lastMove) {
        return makeMove(lastMove, Role.MAX);
    }

    @Override
    public GameMove minimizeCrossings(GameMove lastMove) {
        return makeMove(lastMove, Role.MIN);
    }

    @Override
    public GameMove maximizeCrossingAngles(GameMove lastMove) {
        return makeMove(lastMove, Role.MAX_ANGLE);
    }

    @Override
    public GameMove minimizeCrossingAngles(GameMove lastMove) {
        return makeMove(lastMove, Role.MIN_ANGLE);
    }
    ...
    public void initializeNextRound(Graph g, int width, int height, Role role) {
        ...
        if (this.width * this.height < 10000) {
            this.useSmallBoardStrategy = true;
            this.sampleSize = g.getN();
            this.vertexSampleSize = 3;
        } else {
            this.useSmallBoardStrategy = false;
            this.sampleSize = 500;
            this.vertexSampleSize = 1;
        }
    }
     ...
     */
    public GameMove makeMove(GameMove lastMove, Role role) {
        ...
        GameMove newMove = getMove(lastMove, role);
     ...
     * See the {@code enum MaximizingStrategies} for all maximizing strategies.
     * See the {@code enum MinimizingStrategies} respectively.
     * 
     * @param lastMove the last move made by the opponent, {@code null} if it is the
     *                 first move of the game
     * @param role     {@code NewPlayer.Role} containing information about the
     *                 player's objective (minimizing or maximizing, crossing number
     *                 or angle)
     * @return a valid game move aiding the player objective
     */
    public GameMove getMove(GameMove lastMove, Role role) {
        // if we have a small game board, just use the brute force method
        if (this.useSmallBoardStrategy) {
            return getBruteForceMove(lastMove, role);
        }
        // else get a move according to the current maxStrat
        if (roleIsMax(role)) {
            switch (maxStrat) {
                case BruteForce:
                    return getBruteForceMove(lastMove, role);
                ...
                case Percentage:
                    if (this.r.nextDouble() < this.percentage) {
                        return getBruteForceMove(lastMove, role);
                    } else {
                        return getMirroringMove(lastMove);
                    }
                case Annealing:
                    if ((double) gs.getPlacedVertices().size() / this.g.getN() < this.percentage) {
                        return getMirroringMove(lastMove);
                    } else {
                        return getBruteForceMove(lastMove, role);
                    }
    ...
    /**
     * checks if the given role is a max role, e.g. either Role.MAX or
     * ROLE.MAX_ANGLE
     * 
     * @return
     */
    public boolean roleIsMax(Role role) {
        return (role == Role.MAX || role == Role.MAX_ANGLE);
    }
     ...
     * @param lastMove the last move made by the opponent, {@code null} if it is the
     *                 first move of the game
     * @param role     boolean containing information about the player's objective
     *                 (minimizing or maximizing)
     * @return a valid game move
     ...
     */
    public GameMove getBruteForceMove(GameMove lastMove, Role role) {
        ...
        // unplaced neighbors
        if (!vertices.isEmpty()) {
            v = findBestVertex(vertices, role);
        } else {
        ...
        int bestSquareX = roleIsMax(role) ? minCol : maxCol;
        int bestSquareY = roleIsMax(role) ? minRow : maxRow;
        ...
        int bestTotalCrossingsByVertex = roleIsMax(role) ? Integer.MIN_VALUE : Integer.MAX_VALUE;
     ...
     */
    public Vertex findBestVertex(List<Vertex> vertices, Role role) {
    ...
    public GameMove getRadialStuffMove(GameMove lastMove, Role role) {
        try {
            ...
            while (true) {
                ...
                if (height < width) {
                    x = indexToPlaceVertex % (width / this.fieldPercentage);
                    y = indexToPlaceVertex / (width / this.fieldPercentage);
                } else {
                    x = indexToPlaceVertex / (width / this.fieldPercentage);
                    y = indexToPlaceVertex % (width / this.fieldPercentage);
                }
    ...
    /**
     * The available maximizing strategies for the player
     * 
     ...
     */
    public enum MaximizingStrategy {
        BruteForce, Mirroring, Percentage, Annealing, AnnealingReverse, RadialStuff;
    };

    /**
     * The available minimizing strategies for the player
     * 
     * @param EdgeDirectionMean explanation here
