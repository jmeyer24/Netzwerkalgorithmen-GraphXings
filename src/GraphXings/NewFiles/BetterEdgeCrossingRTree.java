...
public class BetterEdgeCrossingRTree {
     ...
     */
    public void insertAllCoordinates(HashSet<Vertex> placedVertecies) {
    ...
    public double testCoordinate(Vertex vertex, Coordinate coordinateToAdd,
            HashMap<Vertex, Coordinate> mapVertexToCoordinate) {
        mapVertexToCoordinate.put(vertex, coordinateToAdd);
        vertices.add(vertex);
        double crossingsAddedByVertex = calculateIntersections(mapVertexToCoordinate, vertex);
    ...
    public double calculateIntersections(HashMap<Vertex, Coordinate> mapVertexToCoordinate, Vertex v) {
        int x1 = 0;
        int y1 = 0;
        int x2 = 0;
        int y2 = 0;
                ...
                // its neighbor
                    ...
                    if (lowerEdge != null) {
                        Segment lowerSegment = new Segment(mapVertexToCoordinate.get(lowerEdge.getS()),
                                mapVertexToCoordinate.get(lowerEdge.getT()));
                        if (Segment.intersect(thisSegment, lowerSegment)) {
                            crossings+=Segment.squaredCosineOfAngle(thisSegment,lowerSegment);
                            //crossings++;
                            continue;
                        }
                    }
