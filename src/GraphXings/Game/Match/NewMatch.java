...
public class NewMatch {
	 ...
	 * CROSSING_ANGLE - The objective is to maximize the sum of the squared cosines
	 * of crossing angles.
	 * MIXED - Randomly chooses an objective for each game played during the match.
	 */
	public enum MatchType {
		CROSSING_NUMBER, CROSSING_ANGLE, MIXED
	};
	 ...
	 */
	public NewMatch(NewPlayer player1, NewPlayer player2, GameInstanceFactory factory, int bestOutOf,
			long gameTimeLimit, MatchType matchType) {
		this.player1 = player1;
		this.player2 = player2;
		this.bestOutOf = bestOutOf;
		this.factory = factory;
	 ...
	 * @param gameTimeLimit The time limit for each game in the match.
	 * @param matchType     The type of the match to be played.
	 * @param seed          Seed used for the random number generator that selects
	 *                      game objectives if matchType is MIXED.
	 */
	public NewMatch(NewPlayer player1, NewPlayer player2, GameInstanceFactory factory, int bestOutOf,
			long gameTimeLimit, MatchType matchType, long seed) {
		this.player1 = player1;
		this.player2 = player2;
		this.bestOutOf = bestOutOf;
		this.factory = factory;
	...
	public NewMatchResult play() {
		...
		while (!end) {
			...
			NewGame game = new NewGame(gi.getG(), gi.getWidth(), gi.getHeight(), player1, player2, nextObjective,
					gameTimeLimit);
